...
",
                ...
                if let Some(read) = self.read_char(&instr) {
                    ...
                    match instr {
                        '[' => {
                            ...
                            if closer.is_none() {
                                self.crash(format!(
                                    "couldn't find close bracket for {}",
                                    self.branch_token(pos, cflow)
                                ))
                            }
                            ...
                            node.children
                                .push(CompilableNode::new(NodeType::JmpZero(closer_name.clone())));
                            node.children.push(n_node);
                            node.children
                                .push(CompilableNode::new(NodeType::JmpNonZero(opener)));
                            node.children
                                .push(CompilableNode::empty_function(closer_name));
                            pos = closer;
                        }
                        ']' => {
                            ...
                            node.children.push(CompilableNode::new(NodeType::JmpNonZero(
                                self.branch_token(opener, cflow),
                            )));
                        }
                        'l' | 'r' | 'u' | 'd' => {
                            ...
                            if self.branch_exists(&func_name) {
                                node.children
                                    .push(CompilableNode::new(NodeType::Jump(func_name)));
                                break;
                            }
    ...
    pub fn find_function<'a>(
        &self,
        name: &String,
        node: &'a CompilableNode,
    ) -> Option<&'a CompilableNode> {
        match &node.kind {
