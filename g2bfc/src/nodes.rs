...
impl CompilableNode {
    ...
    pub fn compile(&self, program: &Program) -> String {
        ...
        match &self.kind {
            ...
            JmpZero(to) => format!("cmp [eax], 0\nje {}", to),
            JmpNonZero(to) => format!("cmp [eax], 0\njne {}", to),
            AlterCell(amount) => format!("add [eax], {}", amount),
            MovePtr(dir) => format!("add eax, {}", dir.x + dir.y * program.tape_width),
            WriteChar => "putch".to_owned(),
            ReadChar => "getch".to_owned(),
            ExitNode => "jmp _end".to_owned(),
        }
    }
}
